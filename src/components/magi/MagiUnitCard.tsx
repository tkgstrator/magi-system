...
export const MagiUnitCard = ({ unit }: Props) => {
  ...
  const bgColor = isCompleted ? getCardBackgroundColor(unit.vote) : COLORS.red;
  const borderColor = isCompleted
    ? getCardBorderColor(unit.vote)
    : COLORS.redBright;
  ...
  return (
    <>
      ...
      >
        {/* ヘッダー部分 */}
        <div
          className="p-3 md:p-4 border-b-2 transition-colors duration-500"
          style={{ borderColor }}
        >
          <div className="flex justify-between items-center">
            {/* 左側: 名前 + STATUS */}
            <div>
              <h3 className="text-xl md:text-2xl font-bold text-black font-matisse">
                {unit.name}
              </h3>
              {/* TODO: モデル名表示を有効にする場合はコメントアウトを外す */}
        ...
        <div
          ...
          style={{ borderColor }}
        >
          <div className="text-black/60 text-xs font-semibold tracking-wider mb-2">
            CONFIDENCE
          </div>
          {/* プログレスバー */}
      ...
      <AnimatePresence>
        {showModal && (
          <ReasonModal unit={unit} onClose={() => setShowModal(false)} />
        )}
      </AnimatePresence>
    </>
  );

...
// 拒否権が発動しているかチェック
export const hasVetoActivated = (units: MagiUnit[]): boolean => {
  return units.some((u) => u.vote === 'NO' && u.confidence >= 90)
}

// 拒否権を発動したユニットを取得
export const getVetoUnit = (units: MagiUnit[]): MagiUnit | null => {
  return units.find((u) => u.vote === 'NO' && u.confidence >= 90) || null
}
...
export const getFinalResult = (units: MagiUnit[]): MagiFinalResult => {
  ...
  // 拒否権チェック: confidence >= 90 かつ vote === 'NO' の場合は拒否権発動
  if (hasVetoActivated(units)) return 'DENY'

  // 通常の多数決
  const noCount = units.filter((u) => u.vote === 'NO').length
  const yesCount = units.filter((u) => u.vote === 'YES').length

  if (yesCount > noCount) return 'AGREE'
  return 'DENY'
}
